# CREDITS: this file adapted from https://github.com/epicserve/django-base-site.git
# and changed according to specific needs of current project

# This is just file that uses just to run commands.
# Documentation: https://github.com/casey/just
# Use 'docker compose' (v2) as the default prefix for commands. Set these environment variables to an empty string to overwrite
...
@upgrade_python_requirements:
    ...
    {{ python_cmd_prefix }} pip-compile --upgrade --generate-hashes --output-file requirements/prod_lock.txt requirements/prod.in
    {{ python_cmd_prefix }} pip-compile --upgrade --generate-hashes --output-file requirements/dev_lock.txt requirements/dev.in
